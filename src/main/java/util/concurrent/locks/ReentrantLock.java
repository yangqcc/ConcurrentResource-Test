...
import util.concurrent.locks.AbstractQueuedSynchronizer.ConditionObject;
...
public class ReentrantLock implements Lock, java.io.Serializable {
    ...
    /**
     * 只是在获取锁的时候不响应中断，如果在获取锁的时候被中断，任然继续获取
     * 锁，中断标志位不会清除，获取锁后，按照任务自己的设置的中断策略实现中断
     * Acquires the lock.
     * <p>
    ...
    /**
     * 尝试能否获取，能则返回false，不能获取锁并不阻塞当前线程，只是返回false
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        final boolean nonfairTryAcquire(int acquires) {    //非公平获取
            ...
            int c = getState();
            if (c == 0) {    //当前没有线程获取锁，那么尝试能否直接获取，失败直接返回false
        ...
        final int getHoldCount() {     //判断是否当前线程，是的话就直接返回获取次数，因为锁为独占锁，所以state只表示当前线程获取锁次数
    ...
    static final class NonfairSync extends Sync {
         ...
         * acquire on failure.
         */
        final void lock() {    //首先尝试直接获取，如果失败再加入队列
