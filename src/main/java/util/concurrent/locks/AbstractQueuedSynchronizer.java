 ...
 * acquisition order.
 * 但是，如果你需要，或者希望，定义一个tryAcquire或者tryAcquireShared以便内部调用
...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable {
    ...
    final boolean transferForSignal(Node node) { // 将条件队列中的节点放入等待队列中
        ...
        if (ws > 0 || !compareAndSetWaitStatus(p, ws, Node.SIGNAL)) // 如果前一个节点状态为CANCELLED或设设置状态为SIGNAL失败，那么唤醒当前节点
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        public final void await() throws InterruptedException {
            ...
            if (acquireQueued(node, savedState) && interruptMode != THROW_IE) // 如果节点不是中断状态且请求到了锁，将中断模式设为REINTERRUPTA
